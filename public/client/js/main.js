...
$ajax({
   url: `${window.location.origin}/api/add-product-to-cart`, // API endpoint
        type: "POST", // HTTP method
        data: JSON.stringify({ quantity: 1, productId: productId }), // Data to send (as JSON string)
        contentType: "application/json", // Inform the server that we are sending JSON
        dataType: "json", // Expecting JSON response from the server
// Success callback when the request is successful
...
$("#sumCart").text(sum)
 ...
 $.toast({
    ...
    icon: "error",
    loader: false,
    hideAfter: 5000 // Show error message longer
});
function isLogin(){
    return true;
}
